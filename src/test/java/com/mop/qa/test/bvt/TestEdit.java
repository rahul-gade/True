...
//			inst.verifyNameAndTime(remoteDriver, projectName);
	...
	public void editBasicDetails() throws Exception {
		try {
			...
			basicdeal.editBasicDealDetails(remoteDriver, editProjectName, editCompanyName, trueNorthName, editSector,
					subSector, otherSector, source, dealSize, stake, section);
		} catch (Exception e) {
			LOGGER.info(e);
		}
	}
	...
	public void editCompanyInfo() throws Exception {
		try {
			...
			// String editProjectName = rds.getValue("DATA", currentTest,
			// "EditProjectName");
			...
			String section = rds.getValue("DATA", currentTest, "EditSection");
			companyinfo.editCompanyDetails(remoteDriver, editcmpBackground, editcmpInfo, editleader, editshareHolder,
					editpercentage, section);
		} catch (Exception e) {
			LOGGER.info(e);
		}
	}
	...
	public void editIndustryInfo() throws Exception {
		try {
			...
			industryinfo.editIndustryDetails(remoteDriver, projectName, editProjectName, indSize, trgtMktShare,
					last3year, last5year, pred3year, pred5year, competitorName, percent, section, navigate);
		} catch (Exception e) {
			LOGGER.info(e);
		}
	}
