...
public class JRAddCandidate extends PageBase{
	...
	public void navigateLongList(RemoteWebDriver driver, String position, String company) throws Exception {
		...
		for(int i = 1; i<=count; i++) {
			String pos = driver.findElement(By.xpath("(//div[@class='drafts-list ng-star-inserted']["+i+"]//li/div)[1]")).getText();
			String comp = driver.findElement(By.xpath("(//div[@class='drafts-list ng-star-inserted']["+i+"]//li/div)[2]")).getText();
			if(pos.contains(position) && comp.contains(company)) {
				driver.findElement(By.xpath("(//div[@class='drafts-list ng-star-inserted']["+i+"]//li/div)[1]")).click();
				break;
			}
		}
			Thread.sleep(1000);
	...
	public void enterCandidateDetails(RemoteWebDriver driver, String name, String company, String position, String email, String contact) throws Exception {
	...
	public void selectJRWithLonglist(RemoteWebDriver driver, String position, String company) throws Exception {
		...
		for(int i=1;i<count;i++) {
			...
			String comp = driver.findElement(By.xpath("//div[@class='drafts-list ng-star-inserted']["+"]//div[@class='data']")).getText();
			if(pos.contains(position) && comp.contains(company)) {
				if(driver.findElements(By.xpath("//div[@class='drafts-list ng-star-inserted']["+i+"]//div[@class='status']")).size()>0) {
					driver.findElement(By.xpath("//div[@class='drafts-list ng-star-inserted']["+i+"]//div[@class='status' and text()='Longlist']")).click();
				}
			}
		}
	}
