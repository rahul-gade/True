...
public class CompanyInformation extends PageBase{
	...
	int i=1;
	public void enterCompanyDetails(RemoteWebDriver driver, String cmpInfo, String leader, String shareHolder, String percentage, String mandatory) throws Exception {
		if(i==2){
		...
		if(!driver.findElement(By.xpath("//textarea[@formcontrolname='companybg']")).getAttribute("value").isEmpty()){
...
//		driver.findElement(By.xpath(selectLeader.replace("PLACEHOLDER", leader))).click();
	...
	public void editCompanyDetails(RemoteWebDriver driver, String cmpBackground, String cmpInfo, String leader, String shareHolder, String percentage, String section) throws Exception {
		...
		if(!shareHolder.equals("NA")) {
			...
			int len = driver.findElement(By.xpath("//label[text()='Shareholder Name']/parent::span/parent::div/input")).getText().length();
			if(len<=150){
	...
	public void verifyCompanyInfo(RemoteWebDriver driver, String companyName) throws Exception {
		...
		for (int i=1;i<=3;i++){
			String label = driver.findElement(By.xpath("(//div[text()=' Enter the information of the company selected ']/parent::div//div[contains(@class,'field-infix')]//label[contains(@class,'field-label')])["+i+"]")).getText();
			assertTrue("Labels on Company Information page are "+label);
		}
		for (int i=1;i<=2;i++){
				String label = driver.findElement(By.xpath("(//h3[text()='Key Shareholders']/parent::div//label)["+i+"]")).getText();
