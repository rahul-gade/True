...
public class InvestmentHome extends PageBase {
	...
	public void deleteDeal(RemoteWebDriver driver, String projName, String compName, String sect) throws Exception {
		...
		for (int i = 1; i <= size; i++) {
			String pName = driver
					...
					.getText();
			String cName = driver.findElement(By.xpath("(//h3[text()='DRAFTS']/following-sibling::div//h4)[" + i + "]"))
					.getText();
			String sec = driver.findElement(By.xpath("(//h3[text()='DRAFTS']/following-sibling::div//h5)[" + i + "]"))
					.getText();
			...
			if (pName.trim().equalsIgnoreCase(projName) && cName.trim().equalsIgnoreCase(compName)
				...
				driver.findElement(
						By.xpath("(//h3[text()='DRAFTS']/following-sibling::div//div[@class='action']/a)[" + i + "]"))
						.click();
	...
	public void verifyLiveAndPastDeals(RemoteWebDriver driver) throws Exception {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (i == 0) {
				...
				String ownerImage = driver.findElement(By
						.xpath("((//h3[text()='LIVE DEALS']/following-sibling::div/div)[1]//div[@class='img']/img)[1]"))
						.getAttribute("title");
			} else {
				...
				String ownerImage = driver.findElement(By
						.xpath("((//h3[text()='PAST DEALS']/following-sibling::div/div)[1]//div[@class='img']/img)[1]"))
						.getAttribute("title");
			}
			String projectName = driver.findElement(By.xpath(pName)).getText();
			String companyName = driver.findElement(By.xpath(cName)).getText();
			...
			if (!projectName.isEmpty() && !companyName.isEmpty() && !sectorName.isEmpty() && !dealSize.isEmpty()
					&& !ownerImage.isEmpty()) {
				if (i == 0) {
	...
	public void verifyListView(RemoteWebDriver driver) throws Exception {
		...
		Thread.sleep(500);
		if (driver.findElements(By.xpath("//div[@class='card-list ng-star-inserted']")).size() > 0) {
			assertTrue("Deals displayed in List-View on clicking List-View Button.");
		} else {
			assertFalse("Deals displayed in List-View on clicking List-View Button.");
		}
