...
/**
 * TestRunner is the starting point for UAF
 * 
 * @author SAT
 *
 */
...
//	 public static void main(String args[]) {
			...
			for (int j = 0; j < testLists.size(); j++) {
				...
				for (int i = 0; i < suiteTest.size(); i++) {
					...
					if (suiteTest.get(i).getToolName().equalsIgnoreCase("Connected Device")) {
						...
						if (suiteTest.get(i).getLocality().equalsIgnoreCase("Grid")
								|| suiteTest.get(i).getLocality().equalsIgnoreCase("Cloud")) {
						...
						if (executionType.equalsIgnoreCase("Parallel")) {
							if (toolType.equalsIgnoreCase("Selenium")) {
								if (Integer.parseInt(suiteTest.get(i).getNo_Of_Instances()) <= 1) {
									tests2.add(test);
								} else if (((i + 1) < suiteTest.size()) && (suiteTest.get(i).getTestName1()
										.equals(suiteTest.get(i + 1).getTestName1()))) {
									tests.add(test);
	...
	public static List<List<TestParameter>> testList() {
		...
		try {
			...
			InputStream is = Files.newInputStream(Paths.get("./TestRunnerUAF.xls"));
			...
			for (int i = 0; i < totalSheets; i++) {
				...
				for (int count = 1; count <= sheet.getLastRowNum(); count++) {
					...
					// int iterations = Integer.parseInt(iteration.substring(0,
					// iteration.indexOf(".")));
					if (row.getCell(2).toString().equalsIgnoreCase("Yes")) {
						if (!row.getCell(6).toString().equalsIgnoreCase("Connected Device")) {// Get browsername an
																								// platform name
						...
						for (int l = 0; l < count1; l++) {// count as no of instances
							...
							if (row.getCell(6).toString().equalsIgnoreCase("Connected Device")) {
								...
								if (row.getCell(5).toString().equalsIgnoreCase("Grid")
										|| row.getCell(5).toString().equalsIgnoreCase("Cloud")) {
						...
						if (LOGGER.isDebugEnabled()) {
							LOGGER.debug("testnames" + list.size() + "Get" + list.get(0));
						}
						// LOGGER.info("testnames"+list.size()+"Get"+list.get(0));
	...
	public static String getAppProperties(String key) throws IOException {
		...
		try {
			...
			InputStream is = Files.newInputStream(Paths.get("data.properties"));
